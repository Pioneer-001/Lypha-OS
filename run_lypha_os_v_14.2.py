...
# -*- coding: utf-8 -*-
"""
Lypha-OS Kernel v14.2 — TOTAL No-X EDITION (Patched, Path-Hardened)
===================================================================
Pioneer-001 전용 — Pulse Mapping + Z₀ v2 + Linguistic Math Engine 지원 버전

+ 추가 패치 (Path-Hardening + Layer Aliases v14.0 → v14.2 확장):
...
+ v14.2 Judgment Engine 패치:
- Linguistic_Math_Engine_Spec(.en.v1.2 등)을 Math Pulse의 최우선 엔진으로 사용
- context에 evaluation 모드 추가 (가치/선택/티어 판단 상황)
- Cognitive Graph에서 evaluation 모드일 때 Math를 판단 코어로 승격
- Pulse Re-ingest에서 evaluation 모드일 경우 Math 가중치 상향
"""
...
log = lambda m: print(f"[Lypha-OS v14.2] {m}")
...
def auto_patch_Z_and_policy(root: Path, policy: dict, logs: dict) -> dict:
    ...
    zpatch = {
        "patch_source": "V→Z_auto_patch_v14.2",
        "verified": logs,
    }
...
def run_autoboot(root: Path):
    ...
    log("=== Autoboot 완료 (v14.2 Hyper-Init) ===")
...
def main():
    ...
    log("Lypha-OS Kernel v14.2 Start — TOTAL No-X EDITION (Patched, Path-Hardened)")
    ...
    log("Lypha-OS Kernel v14.2 Complete — TOTAL Runtime Active (No-X, Path-Hardened).")
